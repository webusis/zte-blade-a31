...
static void _tcpm_log(struct tcpm_port *port, const char *fmt, va_list args)
{
	...
	scnprintf(port->logbuffer[port->logbuffer_head],
		  LOG_BUFFER_ENTRY_SIZE, "[%5lu.%06lu][tcpm][index = %d] %s",
...
static unsigned int tcpm_pd_select_pps_apdo(struct tcpm_port *port)
{
	 ...
	 * 5V 3A.
	 */
	tcpm_log(port, "%s[line=%d] ->: port->nr_source_caps=%d, port->nr_snk_pdo=%d"
		 , __func__, __LINE__, port->nr_source_caps, port->nr_snk_pdo);
	for (i = 1; i < port->nr_source_caps; ++i) {
		pdo = port->source_caps[i];

		tcpm_log(port, "%s[line=%d] ->: src_pdo_type[%d]=%d", __func__, __LINE__, i, pdo_type(pdo));
		switch (pdo_type(pdo)) {
		case PDO_TYPE_APDO:
			if (pdo_apdo_type(pdo) != APDO_TYPE_PPS) {
			...
			for (j = 1; j < port->nr_snk_pdo; j++) {
				...
				tcpm_log(port, "%s[line=%d] ->: snk_pdo_type[%d]=%d", __func__, __LINE__, j, pdo_type(pdo));
				switch (pdo_type(pdo)) {
				case PDO_TYPE_APDO:
					if (pdo_apdo_type(pdo) != APDO_TYPE_PPS) {
						tcpm_log(port,
				...
				tcpm_log(port, "%s[%d]->:snk_pdo_type[%d]=%d, SRC[%u-%u mV], SNK[%u-%u mV]",
					 __func__, __LINE__, j, pdo_type(pdo),
					 min_src_mv, max_src_mv, min_snk_mv, max_snk_mv);
...
static void run_state_machine(struct tcpm_port *port)
{
	...
	case SNK_HARD_RESET_SINK_OFF:
		 ...
		 */
		tcpm_set_state(port, SNK_HARD_RESET_SINK_ON, PD_T_SAFE_0V + PD_T_SRC_RECOVER_MAX
				+ PD_T_SRC_TURN_ON);
		break;
...
static int tcpm_pps_set_out_volt(struct tcpm_port *port, u16 out_volt)
{
	...
	if (!port->pps_data.active) {
		ret = -EOPNOTSUPP;
		pr_info("xxxx active %d\n",	port->pps_data.active);
		goto port_unlock;
	}

	if (port->state != SNK_READY) {
		ret = -EAGAIN;
		pr_info("xxxx state %d\n", port->state);
		goto port_unlock;
	}
	...
	if (out_volt < port->pps_data.min_volt ||
		...
		ret = -EINVAL;
		pr_info("xxxx min_volt %d, max_volt %d\n",
				port->pps_data.min_volt, port->pps_data.max_volt);
		goto port_unlock;
	}
	...
	if (target_mw < port->operating_snk_mw) {
		ret = -EINVAL;
		pr_info("xxxx target_mw %d < operating_snk_mw %d\n",
				target_mw, port->operating_snk_mw);
		goto port_unlock;
	}
...
sink:
	...
	for (i = 0; i < port->nr_snk_pdo; i++) {
		...
		switch (pdo_type(pdo)) {
		case PDO_TYPE_APDO:
			tcpm_log(port, "%s[line=%d] ->: snk_pdo_type[%d]=%d, %u-%u mV, %u mA",
				 __func__, __LINE__,
				 i, pdo_type(pdo),
				 pdo_pps_apdo_min_voltage(pdo),
				 pdo_pps_apdo_max_voltage(pdo),
			...
			break;
		default:
			tcpm_log(port, "%s[line=%d] ->: snk_pdo_type[%d]=%d, %u mV, %u mA",
				 __func__, __LINE__,
				 i, pdo_type(pdo),
				 pdo_fixed_voltage(pdo),
				 pdo_max_current(pdo));
					      ...
					      struct adapter_power_cap *pd_source_cap)
{
	...
	for (i = 0; i < port->nr_source_caps; i++) {
		...
		case PDO_TYPE_FIXED:
			...
			pd_source_cap->min_mv[i] = pdo_fixed_voltage(pdo);
			     ...
			     const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		if (val->intval > port->pps_data.max_curr * 1000)
			port->pps_data.op_curr = port->pps_data.max_curr * 1000;
		else
			port->pps_data.op_curr = val->intval;
