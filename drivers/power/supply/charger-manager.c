...
static void fullbatt_vchk(struct work_struct *work)
{
	...
	if (charger_policy_get_status() == true) {
		...
		if (cm->policy.battery_charging_disabled_votable &&
			get_client_vote(cm->policy.battery_charging_disabled_votable, OTHER_SETTING_VOTER) == 1) {
			vote(cm->policy.battery_charging_disabled_votable, OTHER_SETTING_VOTER, false, 0);
		}
		return;
	}
...
static int cm_adapt_plug_check(struct charger_manager *cm)
{
	...
	if ((bat_uA < 0) && (charge_vol <= batt_uV) && is_ext_pwr_online(cm)) {
		...
		vote(cm->policy.battery_charging_disabled_votable, UVP_SETTING_VOTER,
											true, 0);
...
static bool _cm_monitor(struct charger_manager *cm)
{
	...
	if (cm->parallel_charge_enabled && !cm->should_en_parallel_chg) {
		...
		rerun_election(policy->battery_charging_disabled_votable);
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		val->intval = get_client_vote(policy->usb_icl_votable, ADB_SETTING_VOTER);
		val->intval = (val->intval < 0) ? 1 : 0;
		break;
	case POWER_SUPPLY_PROP_BATTERY_CHARGING_ENABLED:
		val->intval = get_client_vote(policy->battery_charging_disabled_votable, ADB_SETTING_VOTER);
		val->intval = (val->intval < 0) ? 1 : !val->intval;
		break;
#endif
	default:
		return -EINVAL;
		     ...
		     const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		...
		vote(policy->usb_icl_votable, ADB_SETTING_VOTER, (bool)!val->intval, 0);
		break;
	case POWER_SUPPLY_PROP_BATTERY_CHARGING_ENABLED:
		vote_debug("policy: Set battery charging enable %d\n", val->intval);
		vote(policy->battery_charging_disabled_votable, ADB_SETTING_VOTER,
							!val->intval, 0);
		if (val->intval == 0) {
				...
				union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		...
		break;
	case POWER_SUPPLY_PROP_BATTERY_CHARGING_ENABLED:
		pval->intval = get_client_vote(policy->battery_charging_disabled_votable, POLICY_SETTING_VOTER);
				...
				const union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_BATTERY_CHARGING_ENABLED:
		...
		vote(policy->battery_charging_disabled_votable, POLICY_SETTING_VOTER,
							!pval->intval, 0);
		break;
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT_MAX:
		if (pval->intval < 0) {
...
static int hook_psy_dumper_vote_status(struct charger_manager *cm)
{
	...
	vote_debug("FV: %s[%d], FCC: %s[%d], TOPOFF: %s[%d], "
			...
			get_effective_client(policy->battery_charging_disabled_votable),
			get_effective_result(policy->battery_charging_disabled_votable));
				...
				const union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		...
		vote(policy->battery_charging_disabled_votable, OTHER_SETTING_VOTER, !pval->intval, 0);
...
static int charger_battery_charging_disabled_vote_callback(struct votable *votable,
			void *data, int disable, const char *client)
{
	struct charger_manager *cm = (struct charger_manager *)data;

	vote_debug("callback set disable %d\n", disable);

	charger_set_prop_by_name(cm->desc->psy_hardware,
			POWER_SUPPLY_PROP_STATUS, !disable);
...
#ifdef ZTE_FEATURE_PV_AR
		if (cm->desc->psy_hardware2 && disable)
			charger_set_prop_by_name(cm->desc->psy_hardware2,
...
static int charger_manager_policy_init_default_voter(struct charger_manager *cm)
{
	...
	policy->battery_charging_disabled_votable = create_votable("CHARGING_DISABLE", VOTE_SET_ANY,
					charger_battery_charging_disabled_vote_callback,
					cm);
	if (IS_ERR(policy->battery_charging_disabled_votable)) {
		rc = PTR_ERR(policy->battery_charging_disabled_votable);
		vote_error("create %s failed\n", "battery_charging_disabled_votable");
		goto destroy_votable_loop;
	}
	...
	vote(policy->battery_charging_disabled_votable, DEFAULT_SETTING_VOTER,
										false, 0);
	return 0;
destroy_votable_loop:
	...
	if (policy->battery_charging_disabled_votable)
		destroy_votable(policy->battery_charging_disabled_votable);
