...
static int meminfo_proc_show(struct seq_file *m, void *v)
{
	...
	int lru;
	unsigned long tmp_delta = 15360;
	unsigned long active_file_page = 0;
	...
	active_file_page = pages[LRU_ACTIVE_FILE];

	if (i.totalram < 256000 && available < 87552) {
		available += tmp_delta;
		active_file_page += tmp_delta;
		cached += tmp_delta;
	}
	...
	show_val_kb(m, "Active(file):   ", active_file_page /*pages[LRU_ACTIVE_FILE]*/);
